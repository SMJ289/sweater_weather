...
RSpec.describe DailyWeather do
  ...
  it '.initialize()' do
    ...
    expect(@daily_weather.first.high).to eq(67)
    expect(@daily_weather.first.low).to eq(64)
    expect(@daily_weather.first.type).to eq('Clear')
    expect(@daily_weather.first.icon).to eq('01n')
    expect(@daily_weather.first.rain).to eq(0.96)
    expect(@daily_weather.first.date).to eq("Saturday, Jun 06")
  end
